...
De acordo com a taxonomia utilizada no livro-texto de X [], o DE é uma meta-heurística evolucionária, originalmente desenvolvida para a resolução de problemas de otimização em espaço de buscas contínuo.
Algoritmos classificados como _evolucionários  otimizam um dado problema combinatório através da manutenção das chamadas populações.
...
Visando uma maior fundamentação das conclusões extraídas desse estudo de caso acerca da melhor configuração do DE, optamos pela realização de um experimento estatístico baseado em _blocagem_, detalhada no livro-texto [Livro do Montgomerey].
Além disso, nosso experimento também é baseado nos _guidelines  apresentados no _paper  de Campelo e Takahashi [X].
A seguir, apresentamos a metodologia utilizada no experimento realizado nesse estudo de caso.
...
Os experimentos realizados e detalhados nas seções anteriores indicam que a configuração X performa melhor do que a configuração Y.
...
É importante ressaltar que tais testes são comumente utilizados em estudos estatísticos de algoritmos evolucionários [Y], pois, como presenciamos até aqui no presente estudo de caso, tais algoritmos não costumam apresentar resultados com distribuições bem comportadas.
...
O p-valor extremamente baixo ( $9.9999 \times 10^{-6}$ ) reforça que essa diferença não é fruto do acaso, sendo estatisticamente significativa.
...
Com um p-valor extremamente baixo ($\leq 2.2 \times 10^{-16}$), rejeita-se a Hipótese Nula ($H_0$) de que não há diferença significativa entre as configurações.
